...
`--repo` 로 그 상수가 없는 옛 코드를 돌리면 `AttributeError` 로 시나리오가 통째로 죽었습니다. 차등 검증의
전제를 깨는 결함이라 판정 기준값을 `getattr` 폴백으로 바꿨습니다. 앞으로 새 상수를 판정에 쓸 때는 반드시
같은 방식을 써야 합니다.
...
| 저장소 | 최소 거리 | 측면 속도 | 판정 |
|---|---|---|---|
| 첫 실행 (약한 램프) | 0.65 m | 0.09 m/s | PASS (여유 부족) |
| 현재 (램프 수정) | (미실행) | | |
| 회피 전 da515dd | (미실행 — 하네스 결함으로 실행 불가였음) | | |
