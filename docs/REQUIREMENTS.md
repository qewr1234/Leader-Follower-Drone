...
| OPS-09 | C++ 코어(`cpp/`, pybind11 모듈 `mars_core`)는 IMM-EKF(`ImmEkf`)·제어 법칙과 방벽(`compute_velocity_cmd`·`smooth_velocity_cmd`·`leader_velocity_ff`·`self_velocity_lpf`·`level_fru_by_roll_pitch`·`sanitize_cmd`·`clamp`)·미션 상태머신(`MissionManager`)을 파이썬과 같은 API 로 제공하고, (1) gtest 불변식, (2) 난수 입력 차등 검사(EKF 1e-9, 제어 1e-12, 미션 상태·정책·타이머 완전 일치), (3) `MARS_CORE=cpp` 폐루프 setpoint 스트림이 파이썬 골든과 동일 + 전 시나리오 통과 — 세 겹으로 검증된다. 기본은 파이썬(`MARS_CORE=py`) | cpp/README.md | T | UT[C++ 코어:]; CL[main.main()] | 검증됨 (x86; 최대 편차 EKF 1.2e-14, 제어 2.7e-15, 미션 62 748 스텝 일치. Jetson 빌드는 실기 확인) |
