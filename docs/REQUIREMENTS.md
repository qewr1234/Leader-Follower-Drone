...
| FCR-17 | 리더가 MAX_VX(0.35 m/s)보다 빠르게 다가오면 정면 후퇴로는 벗어날 수 없다. 회피 반경(1.5 m) 안에서는 시선에 수직인 수평 방향으로 비켜서서 접촉을 피한다 | 오프라인 모의: 0.7 m/s 정면 접근 시 회피 없으면 0.01 m(접촉), 있으면 0.41 m | T | UT[회피:]; SITL[min_separation] | 부분 (단위 검증. SITL 미실행) |
...
| FCR-17 | 부분 | `python3 sitl/harness.py --scenario min_separation` 실행. 수정 전 코드(`--repo`, 커밋 da515dd)와 대조하면 접촉 대 0.4 m 차이가 나야 한다 |
