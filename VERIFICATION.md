...
```bash
python3 test_fixes.py                        # 단위 128개, 하드웨어 불필요
...
`python3 test_fixes.py` — 당시 18개 검사 전부 통과(이후 검사가 늘어 현재 128개). 하드웨어 없이 순수 로직만 검증합니다.
...
- **결과** — P+D 단독은 PM 86°, GM 18.6 dB, 스트링 안정. **현재 피드포워드 구현은 GM 4.9 dB(좌우축 4.4 dB), 감도
  피크 2.31, 1.15 rad/s 에서 `|Γ|` 1.80 으로 스트링 불안정**입니다. 원인은 리더 속도 추정 `v_F + v̂_rel` 에서
  FC 자기 속도(지연 0.1 s)와 EKF 상대속도(지연 0.30 s)의 지연 차이만큼 자기 속도가 되먹임되는 것이고, 코드 주석의
  가정(EKF 지연 1 s)과 실측이 달랐습니다. 데드밴드 램프의 국소 기울기 3 도 리더 0.1~0.2 m/s 에서 증폭을 3.5 로 키웁니다.
- **왜 SITL 이 못 잡았나** — 등속 리더(`depth_range`)와 계단 응답은 1.15 rad/s 성분이 작아 공진을 자극하지 않고,
  `MAX_VX` 0.35 포화가 진폭을 깎습니다. 정현파 리더 시나리오가 필요합니다.
- **개선안(코드 미적용)** — 피드포워드 저역통과 0.7 → 2.0 s, 자기 속도에 0.3 s 정합 저역통과, 데드밴드를 기울기 1 인
  소프트 데드존으로. GM 14.4 dB, Ms 1.29, `|Γ|` 1.13 이 되고 정상상태 오차는 그대로입니다. 3대 이상 체인이면
  KFF 0.6(1.02) 또는 ESP32 선두 절대 속도 방송.
- 단위 검사 `분석:` 4개가 기준선 여유, 현재 설계 골든값, 개선안 여유를 회귀로 고정합니다.
...
- **피드포워드의 이득여유·스트링 안정성 미충족** — 위 분석 절. 단독 추종·등속 리더에서는 드러나지 않지만
  리더가 5~6 초 주기로 가감속하거나 기체를 2대 이상 이으면 증폭됩니다. 개선안은 정해져 있고 SITL 정현파
  시나리오로 재현한 뒤 적용해야 합니다.
