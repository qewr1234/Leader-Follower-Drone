...
```bash
python3 test_fixes.py                        # 단위 129개, 하드웨어 불필요
...
python3 sitl/harness.py --all                # SITL 9개 시나리오
...
`python3 test_fixes.py` — 당시 18개 검사 전부 통과(이후 검사가 늘어 현재 129개). 하드웨어 없이 순수 로직만 검증합니다.
...
- **SITL 미실행** — `leader_sine` 은 아직 ArduCopter 에서 돌리지 않았습니다. 현재 코드와 수정 전 코드(커밋 e4b4cd7,
  `--repo`) 로 각각 돌려 진폭비(선형 예측 0.70 / 비선형 대조군 2.3) 를 대조해야 합니다.

## 요구도 추적성 (2026-09-18)

[docs/REQUIREMENTS.md](docs/REQUIREMENTS.md) 에 요구도 57개(FCR 15 · EST 12 · SAF 15 · IF 11 · OPS 4)를 ID 로 두고
...
요구도에 연결돼 있습니다. 미충족 3개(FCR-09 이득여유, FCR-10 스트링 안정성, EST-12 검출률), 미검증 2개(실비행,
ESP32 펌웨어), 부분 6개가 표 3절에 필요한 조치와 함께 있습니다.
...
- **피드포워드 수정의 SITL 확인 대기** — 위 분석 절의 수정은 선형 모델과 비선형 체인 시뮬레이션으로만 검증됐습니다.
  `leader_sine` 을 실제 ArduCopter 에서 현재/수정 전 코드로 대조하기 전까지는 분석 결과입니다. 남은 `|Γ|` 1.13 은
  체인 운용 시 KFF 0.6 으로 내릴지 결정해야 합니다.
