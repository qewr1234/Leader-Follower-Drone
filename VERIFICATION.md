...
간격 3.00 m(수정 전 2.56 m — 오버슈트 없음). SITL `leader_sine` 조건의 비선형 예측은 **0.88** (직전 설계 0.70, 실측
...
- ~~**스트링 안정성 잔여 피크 1.13 (0.27 rad/s)**~~ → 해소(위 절: 절대속도 추정기 + 시간간격 정책 + FF τ 0.1 s, 전 축 ≤ 1.000).
  **SITL `leader_sine`·`depth_range`·`hover_hold` 는 직전 설계로 실측한 값이라 현재 파라미터(Kp 0.30, KV 0.2)로 재실행해야
  합니다.** 예측: leader_sine 진폭비 0.88, depth_range 정상상태 3.53 m.
