...
```bash
python3 test_fixes.py                        # 단위 143개, 하드웨어 불필요
...
`python3 test_fixes.py` — 당시 18개 검사 전부 통과(이후 검사가 늘어 현재 143개). 하드웨어 없이 순수 로직만 검증합니다.
...
[docs/REQUIREMENTS.md](docs/REQUIREMENTS.md) 에 요구도 61개(FCR 16 · EST 15 · SAF 15 · IF 11 · OPS 4)를 ID 로 두고
