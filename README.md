...
>
> C1·C2는 **ArduCopter SITL 실비행으로 차등 검증**되었습니다 — 수정 전 코드에서 재현되고
> 수정 후 코드에서 사라집니다([결과](#sitl-회귀-검증)). C5는 SITL에서 **증상이 재현되지
> 않았습니다**(아래 참조). C3·C4·C6·H2는 단위 테스트만 거쳤습니다.
>
...
| 시나리오 | 수정 후 | 수정 전 (대조군) |
|---|---|---|
| **C1** 리더 미획득 상태로 대기 | ✅ 35초간 GUIDED 유지, LAND 없음 (947프레임) | ❌ **t+0.9초에 LAND 전환** |
| **C2** FAILSAFE_LAND 중 조종사 LOITER 탈환 | ✅ LOITER 25초 유지, 송신 차단 확인 (1073프레임) | ❌ **0.1초 만에 LAND로 뺏김** |
...
C3(공중 착륙 판정) · C4(깊이 범위) · C6(PX4 경로) · H2(호버 정위치 유지)는 단위 테스트만
거쳤습니다. C6은 PX4 SITL이 별도로 필요합니다.
...
1. **남은 항목 SITL 검증** — C3·C4·H2 시나리오 추가, C6은 PX4 SITL 필요
