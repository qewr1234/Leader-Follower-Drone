...
네 축 모두 P+D 에 리더 속도 피드포워드와 자기 속도 감쇠(시간간격 정책)를 더해 제어합니다. 상대 위치·속도와
리더 절대 속도 `v_L` 은 IMM-EKF 추정값(FRU: front/right/up)이고, FC 가 주는 자기 속도 `v_F`(LOCAL_POSITION_NED)는
추정기의 예측 입력이자 감쇠 항의 입력입니다.

| 축 | 명령 | 식 |
|---|---|---|
| 전후 | 거리 유지 | `KFF × v_L,front + KP_FORWARD × (front − TARGET) + KD × v_front − KV × v_F,front` |
| 좌우 | 측면 정렬 | `KFF × v_L,right + KP_RIGHT × right + KD × v_right − KV × v_F,right` |
| 상하 | 고도 정렬 | `KFF × v_L,up + KP_UP × up + KD × v_up − KV × v_F,up` |
| 기수 | 시야 중앙 유지 | `KP_YAW × atan2(right, front)` |

`−KV × v_F` 는 전후축에서 목표 이격을 `3.0 + (KV/KP) × v` 로 속도에 비례해 벌리는 **시간간격 정책**(h ≈ 0.7 s)과
같습니다. 등간격 정책은 선행 기체 정보만으로는 스트링 안정이 되지 않기 때문에 넣었습니다(아래).

추정 불확실성(`pos_cov_trace`)이 크면 전체 명령에 0.55 / 0.75배 감속이 걸립니다.

**피드포워드가 없으면 정상상태 거리 오차가 `v/KP_FORWARD` 로 남습니다** — 초기 설계(KP 0.22)에서 리더 0.3 m/s 에
1.4 m, 1 m/s 에 4.5 m 로 깊이창(10 m) 밖으로 밀려 소실됐고, 초기 SITL 실측 평형 거리 4.3 m 가 이론값
`3.0 + 0.3/0.22 = 4.36 m` 와 일치한 것이 이 오차입니다. 피드포워드(`KFF = 0.8`)와 시간간격 항(`KV = 0.2`)을 넣으면
오차는 `(v − KFF × (v − 0.05) + KV × v)/KP` 로, `KP = 0.30` 에서 0.3 m/s 에 0.53 m 입니다(0.05 는 소프트 데드존).

**피드포워드의 안정성 여유는 선형 모델로 확인했습니다** ([docs/STABILITY_MARGINS.md](docs/STABILITY_MARGINS.md)).
처음 설계는 `v_L` 을 자기 속도 + EKF 상대 속도로 만들었고, 두 항의 지연 차이(EKF 속도 추정 63 % 응답 0.30 s 실측)만큼
자기 속도가 명령에 양성 되먹임되어 이득여유 4.9 dB, 1.15 rad/s 에서 리더 속도 변동을 1.8배 증폭하는 스트링 불안정
설계였습니다. 2026-09-18 에 저역통과로 눌러 14.4 dB / 1.13 으로 만들었고(SITL `leader_sine` 실측 0.72배, 수정 전 코드
1.95배), 2026-09-19 에 추정기가 리더 절대 속도를 직접 추정하도록 바꾸면서 그 경로 자체가 사라졌습니다(이득여유 26.6 dB).
남은 피크 1.14 는 등간격 정책의 고유한 것이라 시간간격 항 `KV` 로 눌렀고, 전 축 **|Γ| ≤ 1.000** 입니다. 데드밴드는
국소 기울기가 3 이던 램프 대신 기울기 1 인 소프트 데드존(0.05 m/s)입니다. 실제 코드로 돌린 4단 체인 시뮬레이션이
선형 예측과 6 % 안에서 맞습니다(현재 파라미터의 SITL 재실행은 아직입니다 — 예측 0.88배).
...
- **카메라 hiccup 내성.** 프레임 드롭은 드롭으로 처리하고, 헤드리스(`MARS_SHOW_WINDOW=0`)
  환경에서도 동작합니다.
- **출발·정지·착륙 판단은 리더의 절대 속도로 합니다.** IMM-EKF 가 자기 속도(FC)를 예측 입력으로 받아 직접
  추정한 절대 속도, ESP32 가 있으면 그 절대 속도. 상대 속도만 보면 후미가 선두 속도를 맞추는 순간 0 이 되어 "선두 정지"로 오판하고,
...
ArduCopter 에서 검사합니다(실측: 현재 코드 0.72배, 수정 전 코드 1.95배로 FAIL). 요구도 62개 중 어느 것이 어느 검사로 검증되고 무엇이 미충족인지는
...
- [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md) — 요구도 62개와 검사·시나리오·분석으로의 추적성 표 (`analysis/trace_check.py` 로 자동 대조)
