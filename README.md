...
`−KV × v_F` 는 전후축에서 목표 이격을 `3.0 + (KV/KP) × v` 로 속도에 비례해 벌리는 **시간간격 정책**(h = 1.0 s)과
...
`3.0 + 0.3/0.22 = 4.36 m` 와 일치한 것이 이 오차입니다. 피드포워드(`KFF = 0.8`)와 시간간격 항(`KV = 0.3`)을 넣으면
오차는 `(v − KFF × (v − 0.05) + KV × v)/KP` 로, `KP = 0.30` 에서 0.3 m/s 에 0.63 m 입니다(0.05 는 소프트 데드존).
...
선형 예측과 7 % 안에서 맞습니다(현재 파라미터의 SITL 재실행은 아직입니다 — 예측 0.82배).
