...
>
> C1·C2·C3·C4·H2는 ArduCopter SITL, C6은 PX4 SITL로 **차등 검증**되었습니다 — 결함이
> 대조군에서 재현되고 수정 후 사라집니다([결과](#sitl-회귀-검증)). C5만 SITL에서 **증상이
> 재현되지 않았습니다**(아래 참조).
>
...
| 수정본 SITL 재검증 | 🔶 C1·C2·C3·C4·C6·H2 완료 / C5만 재현 실패 |
| **실비행** | ⛔ **한 번도 수행된 적 없음** |
...
1. **실비행** — C5를 제외한 모든 결함이 SITL 차등 검증을 통과했습니다. 다음은 프로펠러 제거
   지상 확인 → 저고도 유선 테스트입니다
